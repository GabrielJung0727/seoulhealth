...
function Field({ label, value }: { label: string; value: string | null | undefined }) {
  ...
  return (
    <div className="bg-gray-50 rounded-xl p-4">
      <dt className="text-sm font-bold text-gray-500 mb-1">{label}</dt>
      <dd className="text-lg text-gray-900 whitespace-pre-wrap break-words">{value}</dd>
    </div>
  )
}
          ...
          >
            ...
            >
              ...
              <div className="flex items-center justify-between p-6 border-b border-gray-100 no-print">
                <div>
                  <h2 className="text-2xl font-bold text-gray-900">{item.fullName}</h2>
                  <p className="text-base text-gray-500 mt-1">{toKoreanDateTime(item.createdAt)}</p>
                </div>
              ...
              <div className="px-6 pb-4 no-print">
                <p className="text-base font-bold text-gray-700 mb-3">{L.changeStatus}</p>
                <div className="grid grid-cols-3 gap-3">
                  {(['New', 'Reviewed', 'Contacted'] as Status[]).map((s) => (
                    <button
                      ...
                      className={`min-h-[56px] text-lg font-bold rounded-xl border-2 transition-all ${STATUS_MAP[s].bg} ${STATUS_MAP[s].text} ${
              ...
              <div className="flex gap-3 p-6 border-t border-gray-100 no-print">
                <button
                  ...
                  className="min-h-[48px] px-6 text-base font-bold rounded-xl border-2 border-gray-300 text-gray-700 hover:bg-gray-100 transition-colors"
                >
                  {L.print}
                </button>
                ...
                <button
                  onClick={onClose}
                  className="min-h-[48px] px-8 text-base font-bold rounded-xl bg-brand-navy text-white hover:bg-brand-teal transition-colors"
                >
                  {L.close}
                </button>
              </div>
